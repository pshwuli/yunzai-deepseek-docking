...
export class DeepSeek extends plugin {
    ...
    async messageRouter(e) {
        ...
        // 只有在表情包开关打开时，才执行表情包逻辑
        if (config.enable_emoji && Math.random() < config.emoji_probability) {
            await this.sendMessage(e, 'image');
        }
    }
    ...
    async sendMessage(e, type, content = '') {
        try {
            if (type === 'voice' && config.enable_voice) {
                if (!config.tts_key) return; // 如果语音API key未配置，直接返回
                ...
                if (!config.emoji_api || config.emoji_api.length === 0) return; // 如果表情包API未配置，直接返回
