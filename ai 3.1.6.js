...
// 常量定义
const MESSAGE_COUNT_THRESHOLD = 50; // 群聊消息计数阈值
const MESSAGE_INTERVAL = 5000; // 消息间隔时间（毫秒）
const SEGMENT_DELAY = 500; // 分段发送延迟时间（毫秒）
const RESET_INTERVAL = 3600 * 1000; // 重置计数的时间间隔（毫秒）
const MAX_MESSAGE_HISTORY = 10; // 最大消息历史记录数
const MAX_GROUP_MESSAGES = 100; // 最大群聊消息记录数

const groupMessages = new Map(); // 使用 Map 存储群聊消息记录
const groupCounters = new Map(); // 使用 Map 存储群聊消息计数

const config = {
    enable_master_switch: true, // 总开关（权限最高，关闭后所有功能禁用）
    ...
    emoji_api: ['xxxxx', 'xxxxx', 'xxxxx'], // 表情包 API
    speaker: '酥心御姐', // 说话者角色
    api_url: '', // AI API 地址
    api_key: '', // AI API Key
    tts_key: '', // 语音 API Key
    ...
    standalone_emoji_probability: 0.2, // 单独发送表情包概率
    standalone_voice_probability: 0.2 // 单独发送语音概率
};
...
export class DeepSeek extends plugin {
    ...
    resetCounters() {
        ...
        for (const [group_id, counter] of groupCounters.entries()) {
            if (now - counter.lastResetTime >= RESET_INTERVAL) {
                ...
                counter.lastResetTime = now; // 更新重置时间
    ...
    async messageRouter(e) {
        ...
        if (config.enable_emoji && Math.random() < config.standalone_emoji_probability) await this.sendMessage(e, 'image');
        if (config.enable_voice && Math.random() < config.standalone_voice_probability) await this.sendMessage(e, 'voice', "……");
    }
    ...
    async sendChat(e, prompt, temperature) {
        try {
            ...
            if (config.enable_segment_sending) {
                const segments = response.split(/([。！？])/).filter(s => s.trim() !== '');
